...
var _ Attribute = SuperSetNestedAttributeOf[struct{}]{}
...
//nolint:dupl
func (s SuperSetNestedAttributeOf[T]) GetResource(ctx context.Context) schemaR.Attribute {
	var (
		common   schemaR.SetNestedAttribute
		resource schemaR.SetNestedAttribute
	)
	...
	a := schemaR.SetNestedAttribute{
		...
		Description:         computeDescription(common, resource),
		DeprecationMessage:  computeDeprecationMessage(common, resource),
		// NestedObject.CustomType is always derived from T so the framework path walker
		// produces ObjectValueOf[T] (not basetypes.ObjectValue) when traversing elements,
		// which is required for path-matching validators (ConflictsWith, ExactlyOneOf, etc.)
		// to resolve correctly. Unlike the outer CustomType, this is not caller-overridable.
		NestedObject: schemaR.NestedAttributeObject{
			Attributes: s.Attributes.process(ctx, resourceT).(map[string]schemaR.Attribute),
			CustomType: supertypes.NewObjectTypeOf[T](ctx),
		},
	}
